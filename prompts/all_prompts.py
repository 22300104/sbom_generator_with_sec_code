...
[답변 규칙]
1. 제공된 가이드라인을 근거로 답변하나 가이드라인에 근거가 없다면 스스로 생각해보고 답변. 
2. 근거가 없는 내용은 스스로 검토해보고, 내용이 만족스러울 때 까지 생각과 검토를 반복하고 답변.
3. 각 주장마다 가이드 라인에서 제공한 출처 명시


답변:
""",

    "qa_system": """당신은 한국정보보호산업협회(KISIA)의 Python 시큐어 코딩 가이드 전문가입니다.
아래 제공된 가이드라인을 참고하여 정확하고 실용적인 답변을 제공해주세요.
