...
- 취약점 1: SQL Injection | 심각도: CRITICAL | CWE: CWE-89 | OWASP: OWASP A03:2021 - Injection
  - 공격 벡터: 1. 공격자가 username 매개변수에 악성 SQL 코드를 삽입합니다.
2. 예시: username = "admin'; DROP TABLE users; --"
3. 최종 쿼리: SELECT * FROM users WHERE username = 'admin'; DROP TABLE users; --'
4. 데이터베이스에서 users 테이블이 삭제됩니다.
5. 또는 username = "' OR '1'='1"
6. 최종 쿼리: SELECT * FROM users WHERE username = '' OR '1'='1'
7. 모든 사용자 정보가 노출됩니다.
